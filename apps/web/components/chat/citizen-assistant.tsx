...
export function CitizenAssistant() {
  ...
  return <>
    {pathname !== "/dashboard" && !launcherDismissed ? <div className="fixed right-4 bottom-4 z-40 sm:right-6 sm:bottom-6">
      <Button className="rounded-full bg-primary px-4 shadow-lg shadow-primary/20 transition-transform hover:-translate-y-0.5" onPress={() => { if (messages.length === 0) setMessages([{ id: "welcome", role: "assistant", content: t("chatWelcome") }]); followNextConversationUpdate(); setOpen(true); }} aria-label={t("openAssistant")}><Bot aria-hidden="true" /><span className="hidden sm:inline">{t("assistant")}</span></Button>
      <Button type="button" variant="outline" size="icon-sm" className="absolute -top-2 -right-2 size-6 rounded-full border bg-card/95 text-muted-foreground shadow-sm hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring" onPress={() => setLauncherDismissed(true)} aria-label={t("closeAssistant")}><X className="size-3.5" aria-hidden="true" /></Button>
    </div> : null}
