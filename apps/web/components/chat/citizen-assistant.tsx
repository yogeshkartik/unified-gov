...
export function CitizenAssistant() {
  ...
  return <>
    <Button className="fixed right-4 bottom-4 z-40 rounded-full shadow-lg sm:right-6 sm:bottom-6 sm:hidden" onPress={() => { if (messages.length === 0) setMessages([{ id: "welcome", role: "assistant", content: t("chatWelcome") }]); followNextConversationUpdate(); setOpen(true); }} aria-label={t("openAssistant")}><Bot aria-hidden="true" /></Button>
