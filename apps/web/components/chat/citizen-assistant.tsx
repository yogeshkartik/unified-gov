...
export function CitizenAssistant() {
  ...
  return <>
    {pathname !== "/dashboard" && !launcherDismissed ? <div className="group fixed right-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-40 sm:right-6 sm:bottom-6">
      <Button className="size-14 rounded-full bg-gradient-to-br from-primary via-blue-600 to-indigo-600 p-0 shadow-[0_10px_24px_oklch(0.47_0.17_257_/_28%)] transition-[transform,box-shadow,filter] duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_28px_oklch(0.58_0.15_257_/_36%)] hover:brightness-110 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transform-none sm:h-12 sm:w-auto sm:px-3" onPress={() => { if (messages.length === 0) setMessages([{ id: "welcome", role: "assistant", content: t("chatWelcome") }]); followNextConversationUpdate(); setOpen(true); }} aria-label={t("openAssistant")}><span className="grid size-9 place-items-center rounded-full bg-gradient-to-br from-cyan-300/75 via-blue-300/60 to-violet-300/75 shadow-inner ring-1 ring-white/30"><Sparkles className="size-4 text-white drop-shadow-sm" aria-hidden="true" /></span><span className="hidden sm:inline">{t("assistant")}</span></Button>
      <Button type="button" variant="outline" size="icon" className="absolute -top-3 -right-3 size-7 rounded-full border bg-card/95 text-muted-foreground shadow-sm transition-[opacity,transform,background-color] duration-150 hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring sm:pointer-events-none sm:scale-90 sm:opacity-0 sm:group-hover:pointer-events-auto sm:group-hover:scale-100 sm:group-hover:opacity-100 sm:group-focus-within:pointer-events-auto sm:group-focus-within:scale-100 sm:group-focus-within:opacity-100" onPress={() => setLauncherDismissed(true)} aria-label={t("hideAssistantShortcut")}><X className="size-3.5" aria-hidden="true" /></Button>
    </div> : null}
