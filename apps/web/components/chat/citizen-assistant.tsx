...
import type { ChatServiceCard } from "@/src/types";
...
type Message = { id: string; role: "user" | "assistant"; content: string; components?: ChatServiceCard[] };

function ServiceCard({ service }: { service: ChatServiceCard }) {
  ...
  return <article className="mt-2 rounded-lg border bg-card p-3 text-foreground"><p className="font-semibold">{service.name}</p><p className="mt-1 text-xs text-muted-foreground">{service.department} · {service.category}</p><p className="mt-2 text-sm text-muted-foreground">{service.description}</p><p className="mt-3 text-sm font-medium">{t("fee", { amount: fee })}</p><Link className="mt-3 inline-flex text-sm font-medium text-primary underline-offset-4 hover:underline" href={`/services/${service.service_id}`}>{t("viewDetails")}</Link></article>;
}
...
export function CitizenAssistant() {
  ...
  async function send(value = draft) {
    ...
    try { const response = await api.sendChat(text, history, language); setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant", content: response.message, components: response.components }]); }
    catch { setError(true); }
