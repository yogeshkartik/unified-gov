...
function ApplicationRow({ application, onOpen, onViewDetails, onDelete }: { application: ApplicationListItem; onOpen: (application: ApplicationListItem) => void; onViewDetails: (application: ApplicationListItem) => void; onDelete: (application: ApplicationListItem) => void }) {
  ...
  return <article className={`grid gap-3 px-5 py-5 transition-colors hover:bg-muted/40 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto_auto] sm:items-center sm:gap-6 ${draft ? "bg-amber-50/30" : ""}`}>
    ...
    <div className="flex items-baseline justify-between gap-3 sm:block"><p className="text-xs text-muted-foreground sm:hidden">Fee</p><p className="text-sm font-medium">{application.fee > 0 ? `${application.currency} ${application.fee}` : "Free"}</p></div>
...
function ApplicationList({ applications, onOpen, onViewDetails, onDelete }: { applications: ApplicationListItem[]; onOpen: (application: ApplicationListItem) => void; onViewDetails: (application: ApplicationListItem) => void; onDelete: (application: ApplicationListItem) => void }) {
  return <section className="overflow-hidden rounded-xl border bg-card shadow-sm" aria-label="Applications"><div className="hidden grid-cols-[minmax(0,1fr)_auto_auto_auto_auto] gap-6 border-b px-5 py-3 text-xs font-medium text-muted-foreground sm:grid"><span>Application</span><span>Status</span><span>Updated</span><span>Fee</span><span className="sr-only">Action</span></div>{applications.map((application, index) => <div key={application.id} className={index > 0 ? "border-t" : undefined}><ApplicationRow application={application} onOpen={onOpen} onViewDetails={onViewDetails} onDelete={onDelete} /></div>)}</section>;
}
