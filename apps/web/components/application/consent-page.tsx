  ...
  return (
    ...
    >
      ...
      <Dialog isOpen={Boolean(uploadRequirement)} onOpenChange={(open) => { if (!open) { setUploadRequirement(undefined); setUploadFile(undefined); } }}>
        ...
        <div className="space-y-4"><div><p className="text-sm font-medium">{t("documentType")}</p><div className="mt-1 rounded-md border bg-muted/40 px-3 py-2 text-sm"><p>{uploadRequirement?.label}</p><p className="mt-0.5 text-xs text-muted-foreground">{t("requiredForApplication")}</p></div></div><DocumentFilePicker id="required-document-file" file={uploadFile} onChange={setUploadFile} accept={uploadRequirement?.document_type === "PHOTOGRAPH" ? ".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" : undefined} acceptedExtensions={uploadRequirement?.document_type === "PHOTOGRAPH" ? ["jpg", "jpeg", "png", "webp"] : undefined} unsupportedMessage={uploadRequirement?.document_type === "PHOTOGRAPH" ? t("choosePhotoValidation") : undefined} /></div>
