...
export function PortalAccess({ children }: { children: ReactNode }) {
  ...
  if (!ready) return <LoadingState label={t("checkingSession")} />;
  if (!session && publicServiceRoute) return <div className="min-h-screen"><header className="flex min-h-16 items-center justify-between border-b border-primary/10 bg-card px-4 sm:px-8"><Link href="/" className="flex items-center gap-2 font-semibold"><span className="grid size-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Landmark className="size-4" /></span>{t("appShortName")}</Link><LinkButton href="/login" size="sm">{t("signIn")}</LinkButton></header><main className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8">{children}</main></div>;
  if (!session) return <LoadingState label={t("redirectingSignIn")} />;
