  ...
  return (
    <div className={`overflow-hidden transition-[max-height,opacity] duration-200 ease-out motion-reduce:transition-none ${leaving ? "max-h-0 opacity-0" : "max-h-16 opacity-100"}`}>
      <div className={`mx-auto w-full max-w-7xl px-4 pt-3 transition-transform duration-200 motion-reduce:transition-none sm:px-6 lg:px-8 ${leaving ? "-translate-y-1" : "translate-y-0"}`}>
        <div className="flex min-h-10 items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-950 shadow-sm" role="status">
          <Info className="size-4 shrink-0 text-primary" aria-hidden="true" />
          <span>Demo account · Synthetic data only</span>
          <Button type="button" variant="ghost" size="icon-xs" className="-mr-1 ml-auto text-blue-800" aria-label="Dismiss demo notice" onPress={() => setLeaving(true)}>
            <X aria-hidden="true" />
          </Button>
        </div>
      </div>
