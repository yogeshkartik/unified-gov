...
@router.get("/digilocker/documents/{document_id}/file")
@router.get("/digilocker/documents/{document_id}/download")
def download_digilocker_document(document_id: str) -> FileResponse:
    try:
        provider_document = digilocker_service.get_document(document_id)
    except ProviderDocumentNotFoundError as error:
        raise digilocker_document_not_found(document_id) from error
...
def select_digilocker_documents(
    ...
    try:
        return digilocker_service.select_application_documents(
            db, application_id, payload.document_ids
        )
...
def digilocker_document_not_found(document_id: str) -> HTTPException:
    return HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail={
            "code": "DIGILOCKER_DOCUMENT_NOT_FOUND",
            "message": f"DigiLocker document '{document_id}' was not found.",
        },
    )
