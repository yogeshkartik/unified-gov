...
def get_profile(db: Session) -> Profile:
    profile = db.scalar(
        ...
        .options(
            selectinload(Profile.user).selectinload(User.addresses),
            selectinload(Profile.user).selectinload(User.documents),
        )
    )
    if profile is None:
        raise ProfileNotFoundError
...
def delete_document(db: Session, document_id: str) -> None:
    ...
    # Application snapshots already contain immutable document metadata.  Remove
    # live attachment rows so a reusable document can be deleted without
    # changing a submitted application's saved snapshot.
...
def canonical_document_type(document_type: DocumentType) -> DocumentType:
    return DocumentType.PHOTOGRAPH if document_type == DocumentType.PROFILE_PHOTO else document_type


def is_image_upload(content: bytes, extension: str) -> bool:
    signatures = {
        ".jpg": b"\xff\xd8\xff",
        ".jpeg": b"\xff\xd8\xff",
        ".png": b"\x89PNG\r\n\x1a\n",
