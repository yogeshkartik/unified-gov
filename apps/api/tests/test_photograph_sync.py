...
from app.api.documents import upload_application_document
...
def test_deleting_an_attached_reusable_document_detaches_it_first(db: Session) -> None:
    ...
    db.add(signature)
    db.flush()
    attached_ids = {item.document_id for item in get_application(db, application.id).documents}
    db.add_all([
        ApplicationDocument(application_id=application.id, document_id=document_id)
        for document_id in (photograph.id, signature.id)
...
def test_application_photograph_upload_persists_and_attaches(db: Session) -> None:
    seeded_photo = get_profile(db).profile_photo
    assert seeded_photo is not None
    delete_document(db, seeded_photo.id)
    application = create_application(db, "DRIVING_LICENCE_001")
    requirement = next(
        item for item in get_application(db, application.id).service.document_requirements
        if item.document_type == DocumentType.PHOTOGRAPH
    )

    photo = asyncio.run(
        upload_application_document(application.id, requirement.id, image_upload(), db)
    )

    attached = get_application(db, application.id).documents
    assert photo.document_type == DocumentType.PHOTOGRAPH
    assert photo.id in {item.document_id for item in attached}
    _, missing_documents, _ = determine_missing_requirements(db, get_application(db, application.id))
    assert "PHOTOGRAPH" not in missing_documents
    assert photo.id in {item.id for item in list_documents(db)}

    replacement = asyncio.run(
        upload_application_document(application.id, requirement.id, image_upload("replacement.png"), db)
    )
    assert replacement.id == photo.id
    assert db.scalar(
        select(func.count()).select_from(ApplicationDocument).where(
            ApplicationDocument.application_id == application.id,
            ApplicationDocument.document_id == photo.id,
        )
    ) == 1


def test_application_photograph_upload_rejects_pdf(db: Session) -> None:
    seeded_photo = get_profile(db).profile_photo
    assert seeded_photo is not None
    delete_document(db, seeded_photo.id)
    application = create_application(db, "DRIVING_LICENCE_001")
    ...
    with pytest.raises(HTTPException) as error:
        asyncio.run(upload_application_document(application.id, requirement.id, pdf, db))
    assert error.value.status_code == 422
    assert error.value.detail["code"] == "UNSUPPORTED_PHOTOGRAPH"
